...
void CSession::AsyncReadHead(int total_len)
{
	...
	asyncReadFull(HEAD_TOTAL_LEN, [self, this](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			memcpy(_recv_head_node->_data, _data, bytes_transfered);//将_data中的数据复制到_recv_head_node->_data中
...
{
	auto self = shared_from_this();
	_socket.async_read_some(boost::asio::buffer(_data + read_len, total_len - read_len),//写到_data中
		[read_len, total_len, handler, self](const boost::system::error_code& ec, std::size_t  bytesTransfered) {//回调函数便于异步执行，C++11中可以未命名用lamda表达式创建回调函数
			if (ec) {
				// 出现错误，调用回调函数
